...
namespace CSFramework {
	public class ShortcutToolEditorWindow : CustomSlotEditorWindow {
		...
		private void OnGUI() {
            ...
            GUILayout.Space(10);
            if (GUILayout.Button("Save Loadout" ) && !EditorApplication.isPlaying) {
                slot.symbolGen.SaveLoadout();
            }
            GUILayout.Space(10);
            if (GUILayout.Button("Load Loadout") && !EditorApplication.isPlaying) {
                slot.symbolGen.LoadLoadout();
            }
