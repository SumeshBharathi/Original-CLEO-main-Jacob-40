...
namespace CSFramework
{
	...
	public class CustomSlot : MonoBehaviour
	{
		...
		public void Initialize()
		{
			...
			if (debug.alwaysMaxLines) lineManager.SwitchAllLines(false, true);
			else lineManager.SwitchAllLines(false, true);
		...
		public void StartSpin(float duration = 0)
		{
			...
			if (config.advanced.stopReelsWithIndex)
			{
				int freeSpinOffset = gameInfo.totalFreeSpins * config.advanced.freeSpinSpinOffset;
				for (int i = 0; i < config.reelLength; i++)
				{
					int multiplyer = (
						5 - config.hiddenTopRows
						+ config.reel.initialReelPosition
						+ ((gameInfo.roundsCompleted + 1) * (config.advanced.defaultSpinOffset - config.reel.initialReelPosition))
						+ (i * (gameInfo.roundsCompleted + 1) * config.advanced.reelOffset)
						+ freeSpinOffset
						) % config.symbolsPerReel;
					reels[i].SetManipulation(null, multiplyer, 1);
				}
			}
		...
		internal void ProcessHit(HitInfo info)
		{
			...
			if (info.hitSymbol.payType == Symbol.PayType.Normal)
			{
				...
				//for logging
				gameInfo.payoutsSequence.Append(info.hitChains + ";");
				gameInfo.addBalanceSequence.Append((info.payout * gameInfo.bet) + ";");
				gameInfo.hitSymbolSequence.Append(info.hitSymbol + ";");
			}
		...
		public void Refresh() { layout.Refresh(true); }
